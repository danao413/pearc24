...
def split_data(
    ...
    #another option is to just pass the "training_parameters" here 
    dataset_size = len(dataset)
    #unpack the training parameters here
    ...
    split = training_parameters['random_split']
    split_path = training_parameters['split_path']
    ...
    if (train_ratio + val_ratio + test_ratio) <= 1:
        ...
        #can we get the data processing from below? or the same path?
        elif split == 'False':
            print('I got to the right place')
            #get the indices
            with open(split_path, 'r') as f:
                index_data = json.load(f)
                train_idx = np.array(index_data['train']).astype('int')
                val_idx = np.array(index_data['val']).astype('int')
                test_idx = np.array(index_data['test']).astype('int')
            f.close()
            #now split the data using the same indices every time
            train_dataset = torch.utils.data.Subset(dataset, indices=torch.from_numpy(train_idx))
            val_dataset = torch.utils.data.Subset(dataset, indices=torch.from_numpy(val_idx))
            test_dataset = torch.utils.data.Subset(dataset, indices=torch.from_numpy(test_idx))
