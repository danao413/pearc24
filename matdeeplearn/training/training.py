...
def trainer(
    ...
    best_val_error = 1e10
    model_best = model
    ##we want to load the dictionary if the file already exists
    if os.path.exists(error_file):
        with open(error_file, 'r') as f:
            error_dict = json.load(f)
        f.close()
        num_epochs = list(error_dict['train'].keys())
        last_epoch = int(num_epochs[-1])
    else:
    ...
    for epoch in range(last_epoch + 1, epochs + 1):
        ...
        ##Train model
        #this is the loss NOT the error
        ...
        #get the training/val accuracy
        if val_loader != None and rank in (0, 'cpu', 'cuda'):
            if rank not in ('cpu', 'cuda'):
                train_loss, train_out = evaluate(train_loader, model.module, loss, rank=rank, out=True)
                val_loss, val_out = evaluate(val_loader, model.module, loss, rank=rank, out=True)
            else:
                train_loss, train_out = evaluate(train_loader, model, loss, rank=rank, out=True)
                val_loss, val_out = evaluate(val_loader, model, loss, rank=rank, out=True)
        ...
        #put the val_error and train_error in the dictionary
        train_arr = train_error.detach().cpu().numpy()
        val_arr = val_error.detach().cpu().numpy()
        error_dict['train'][epoch] = float(train_arr)
        error_dict['val'][epoch] = float(val_arr)
    ...
    #there is a default filename here, will want to include a new argument in the config file so that we can run multiple jobs
    return model_best
...
#change this to "training_parameters"
def loader_setup(
    training_parameters,
    batch_size,
    dataset,
    ...
    train_dataset, val_dataset, test_dataset = process.split_data(
        dataset, training_parameters, seed
    )
...
def train_regular(
    ...
    ##Set up loader
    #pass the indices here
    #another option: change the loader_setup arguments
    #to be "training_parameters"
    (
        train_loader,
        val_loader,
        test_loader,
    ...
    #get the train/val error file
    model_job_name = model_parameters['model']
    error_file = '{}_{}.json'.format(model_job_name, job_parameters['train_val_file'])
    error_file = os.path.abspath(error_file)

    #ah I see the problem here, they hard coded the file name
    #but why isn't the error file being written either?
    model_save_path = job_parameters['model_path']
